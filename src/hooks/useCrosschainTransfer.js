...
export function useCrossChainTransfer() {
  ...
  const getSolanaKeypair = (privateKey) => {
    try {
  ...
  const getPrivateKeyForChain = (chainId) => {
  ...
  const getClients = (chainId) => {
    ...
    const account = privateKeyToAccount(`0x${privateKey.replace(/^0x/, "")}`, { nonceManager });
  ...
  const getSolanaBalance = async (chainId) => {
    const connection = getSolanaConnection();
    const privateKey = getPrivateKeyForChain(chainId);
    const keypair = getSolanaKeypair(privateKey);
    const usdcMint = new PublicKey(CHAIN_IDS_TO_USDC_ADDRESSES[chainId]);
  ...
  const getEVMBalance = async (chainId) => {
  ...
  const approveSolanaUSDC = async (keypair, sourceChainId) => {
    setCurrentStep("approving");
    ...
    try {
      ...
      if (isSolanaChain(destinationChainId)) {
        ...
        const usdcMint = new PublicKey(
          CHAIN_IDS_TO_USDC_ADDRESSES[SupportedChainId.SOLANA_DEVNET]
        );
    ...
    try {
      ...
      for (const [chainId, usdcAddress] of Object.entries(
        ...
        if (
          ...
          remoteTokenAddressHex = evmAddressToBytes32(usdcAddress);
          break;
        }
      }
  ...
  const executeTransfer = async (
    ...
    try {
      ...
      let sourceClient, destinationClient, defaultDestination;
      ...
      const checkNativeBalance = async (chainId) => {
        if (isSolanaChain(chainId)) {
          const connection = getSolanaConnection();
          const privateKey = getPrivateKeyForChain(chainId);
          const keypair = getSolanaKeypair(privateKey);
